...
namespace My
{
    public class MySNAuthority
    {
        ...
        /// <param name="bytes">位元數(10 or 16 進位)</param>
        /// <param name="length">字串長度</param>
        /// <param name="chars">不足位數補字元</param>
        /// <returns></returns>
        public string GetRandomSN(int min, int max, int bytes, int length,char chars)
        {
            string str_random;
            Random rnd = new Random();
            long number = rnd.Next(min, max);
            str_random = Convert.ToString(number, bytes).ToUpper().PadLeft(length, chars);
